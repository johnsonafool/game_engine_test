...
import { StateHandler } from "../../tutorial-babylonjs-server/src/rooms/StateHandler";
...
async function main() {
  ...
  box.material["emissiveTexture"] = new BABYLON.Texture(
    "./src/grass.png",
    scene
  );
  ...
  client.joinOrCreate<StateHandler>("game").then((room) => {
    ...
    room.state.players.onAdd = function (player, key) {
      ...
      if (key === room.sessionId) {
        ...
        playerViews[key].physicsImpostor = new BABYLON.PhysicsImpostor(
          playerViews[key],
        ...
        playerViews[key].material = new BABYLON.StandardMaterial(
          "s-mat",
          scene
        );
        playerViews[key].material["diffuseColor"] = new BABYLON.Color3(1, 0, 0);
        playerViews[key].material["emissiveTexture"] = new BABYLON.Texture(
          "./src/grass.png",
          scene
        );
    ...
    window.addEventListener("keydown", function (e) {
      ...
      playerViews[sessionId].physicsImpostor.setLinearVelocity(
        new BABYLON.Vector3(keyboard.x, 0, keyboard.y)
      );
    });
